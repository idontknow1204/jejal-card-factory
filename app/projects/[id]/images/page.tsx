...
async function loadData(projectId: string): Promise<{
  ...
  try {
    ...
    const meta = project
      ? `${project.content_type.toUpperCase()} · ${cards.length} cards · gpt-image-1 via Factchat`
      : null;
...
export default async function ImagesPage({
  ...
  const { projectTitle, projectMeta, cards, images, error } = await loadData(id);
    ...
    <div className="min-h-screen bg-[#FAFAFA]">
      <header className="bg-white border-b border-[#E5E5E5] px-6 py-4">
        <div className="max-w-5xl mx-auto flex items-center gap-3">
          ...
          <span className="font-bold text-[#111111] text-sm">4. Images</span>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-6 py-10 space-y-6">
        <div>
          <h1 className="text-2xl font-extrabold text-[#111111]">
            {projectTitle ? `${projectTitle} — Images` : "Character Images"}
          </h1>
          <p className="text-[#9A9A9A] text-sm mt-1">
            {projectMeta ??
              "Generate and review JeJal character images — character_bible.md §13/§14"}
          </p>
        </div>
        ...
        {cards.length > 0 && (
          <ImagePanel projectId={id} cards={cards} initialImages={images} />
        )}
      </main>
    </div>
  );
